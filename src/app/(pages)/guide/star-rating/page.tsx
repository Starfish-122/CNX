...
export default function StarRatingGuide(): React.JSX.Element {
  ...
  return (
    <GuideLayout
      ...
      description={
        <>
          <CodeTag>StarRating</CodeTag>은 0.5 단위 선택을 지원하는 별점 컴포넌트입니다. 키보드 및 마우스(호버/클릭) 인터랙션을 지원하며,&nbsp;
...
<StarRating value={rating} onChange={setRating} />`}
      ...
      <GuideSection title="정수 단위만 (allowHalf = false)">
        <ExampleCard
          demo={
            <div className="flex items-center gap-4">
              <StarRating value={ratingInteger} onChange={setRatingInteger} allowHalf={false} />
              <Text className="text-sm text-gray-600 dark:text-gray-400">
                현재 값: {ratingInteger}
              </Text>
            </div>
        ...
        <ExampleCard
          demo={
            <ul className="list-disc pl-5 space-y-2 text-gray-700 dark:text-gray-300">
              <li>키보드: <CodeTag>←/→/↑/↓</CodeTag>로 증감, <CodeTag>Home</CodeTag>=0, <CodeTag>End</CodeTag>=max</li>
      ...
      <GuideSection title="Props">
        <PropsTable
          rows={[
            { prop: 'value', type: 'number', def: '—', desc: '현재 별점 (0 ~ max), 0.5 단위 지원' },
            { prop: 'max', type: 'number', def: '5', desc: '최대 별 개수' },
            { prop: 'readOnly', type: 'boolean', def: 'false', desc: '읽기 전용 (입력 비활성화)' },
            { prop: 'disabled', type: 'boolean', def: 'false', desc: '비활성화' },
            { prop: 'size', type: `'sm' | 'md' | 'lg'`, def: `'md'`, desc: '아이콘 크기 프리셋' },
            { prop: 'allowHalf', type: 'boolean', def: 'true', desc: '반개 단위 허용 여부부' },
            { prop: 'onChange', type: '(nextValue: number) => void', def: '—', desc: '값 변경 시 호출되는 콜백함수' },
            { prop: 'className', type: 'string', def: '—', desc: '커스텀 클래스' },
            { prop: 'ariaLabel', type: 'string', def: `'별점'`, desc: '스크린 리더용 레이블' },
          ]}
        />
      </GuideSection>
    </GuideLayout>
